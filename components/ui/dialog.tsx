...
>(({ className, children, ...props }, ref) => {
  return (
    <DialogPrimitive.Overlay
      style={StyleSheet.absoluteFill}
      className={cn('z-50 flex bg-black/60 justify-center items-center p-2', className)}
      {...props}
      ref={ref}
    >
    ...
    <DialogPortal hostName={portalHost}>
      <DialogOverlay>
        <DialogPrimitive.Content
          ref={ref}
          className={cn(
            'z-50 gap-4 border border-border web:cursor-default bg-background p-6 shadow-lg web:duration-200 rounded-xl',
            open
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn(
      'text-lg native:text-lg text-foreground font-semibold leading-none tracking-tight',
      className
    )}
    {...props}
  />
